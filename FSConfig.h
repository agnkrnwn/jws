...
void LoadDataAwal() {
  ...
  if (strlen(configinfo.nama) == 0) {
    strlcpy(configinfo.nama, "NAMA MASJID ", sizeof(configinfo.nama));
  }

  if (strlen(configinfo.info1) == 0) {
    strlcpy(configinfo.info1, "info satu", sizeof(configinfo.info1));
  }

  if (strlen(configinfo.info2) == 0) {
    strlcpy(configinfo.info2, "INFO DUA", sizeof(configinfo.info2));
  }

  if (strlen(configwifi.wifissid) == 0) {
    strlcpy(configwifi.wifissid, "WIFI WIFI", sizeof(configwifi.wifissid));
  }

  if (strlen(configwifi.ssid) == 0) {
    strlcpy(configwifi.ssid, "JWS OWOS", sizeof(configwifi.ssid));
  }
...
void loadInfoConfig(const char *fileconfiginfo, ConfigInfo &configinfo) {  
  ...
  strlcpy(configinfo.nama, doc["nama"] | "INI ADALAH NAMA", sizeof(configinfo.nama));
...
void loadWifiConfig(const char *fileconfigwifi, ConfigWifi &configwifi) {  
  ...
  strlcpy(configwifi.wifissid, doc["wifissid"] | "WIFI", sizeof(configwifi.wifissid));
  strlcpy(configwifi.wifipassword, doc["wifipassword"] | "12345", sizeof(configwifi.wifipassword));
  strlcpy(configwifi.ssid, doc["ssid"] | "WIFI", sizeof(configwifi.wifissid));
